...
def create_proxyauth_extension(proxy_host, proxy_port, proxy_username, proxy_password, scheme='http', plugin_path=None):
    if plugin_path is None:
        plugin_path = 'C:\\reporting\\reporting_chrome_proxyauth_plugin.zip'
...
def report_old_version(browser, actions, keyword):
    ...
    if ('RS' in actions) and ('SS' not in actions):  #
        ...
        while not last_page_checked:
            try:
                ...
                print("[#] Clicking Mark as Read Button")
    ...
    if ('AC' in actions) or ('CL' in actions):
        ...
        keyword_link_ac = WebDriverWait(browser, wait_timeout).until(lambda driver: "%s%s%s%s" % (
            str(browser.current_url)[:str(browser.current_url).index('.com')],
            '.com/?fid=flsearch&srch=1&skws=',
            keyword,
            '&sdr=4&satt=0'
        ))
...
def select_all_msgs_new(browser):
    ...
    WebDriverWait(browser, wait_timeout). \
        until(ec.presence_of_all_elements_located((By.XPATH, '//*[@id="primaryContainer"]/div[4]/div/div[1]/div[2]'
                                                             '/div[5]/div[2]/div[1]/div/div/div[3]/button')))
    print("[-] Getting All Msgs checkbox")
    wait_for_page(browser)
    chk_bx_bttn = WebDriverWait(browser, wait_timeout). \
        until(lambda driver: browser.find_element_by_xpath('//*[@id="primaryContainer"]/div[4]/div/div[1]/div[2]/div[5]'
    ...
    WebDriverWait(browser, wait_timeout). \
        until(ec.visibility_of_element_located((By.XPATH, '//*[@id="primaryContainer"]/div[4]/div/div[1]/div[2]/div[5]'
                                                          '/div[2]/div[1]/div/div/div[3]/button')))
    print("[#] Clicking Checkbox")
    chk_bx_bttn.find_element_by_tag_name("button").click()
    wait_for_page(browser)
    print("[!] End select all messages")
...
def click_link_new(browser):
    try:
        wait_for_page(browser)
        print("[-] Getting the Mail 'Body'")
        body = WebDriverWait(browser, wait_timeout). \
            until(lambda driver: browser.find_element_by_id('Item.MessageUniqueBody'))
        try:
            print("[-] Getting the Link in the Mail !")
            ...
            try:
                ...
                try:
                    if email_language == "English":
                        ...
                        print("Waiting for action to be performed")
                        WebDriverWait(browser, wait_timeout). \
                            until(lambda driver: browser.find_element_by_xpath('//*[@id="primaryContainer"]/div[4]'
                                                                               '/div/div[1]/div[2]/div[5]/div[2]/div[1]'
                                                                               '/div/div/div[3]/div/div[2]/div[1]/span')
                                  .text == "Junk Email")
...
def report_hotmail(**kwargs):
    ...
    if proxy_args is not None:
        proxy_ip = proxy_args['ip_address']
        proxy_port = str(proxy_args['ip_port'])
        proxy_login = proxy_args['proxy_user']
        proxy_pswd = proxy_args['proxy_pswd']
    else:
        proxy_ip = None
        proxy_port = None
        proxy_login = None
    ...
    print("\n******\nStarting JOB for :\n*-Actions: %s\n*-Subject: %s\n*-Email: %s\n*-Password: %s\n*-Proxy: %s\n*-"
          "Port: %s\n******\n" % (kwargs.get('actions', None), subject, mail, pswd, proxy_ip, proxy_port))
